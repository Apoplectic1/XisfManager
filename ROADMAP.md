...
6. **Wire `XisfBlockCompression.Decompress` into a runtime path** — the codec is compress-only today because XFM treats image blocks as opaque. Needed when the flux feature gains real pixel read/write; at that point re-evaluate `Astronomy.PCL` for true image decode/encode (the codec here only covers the byte-block layer).
7. **Compress thumbnail/ICC blocks** — only the main image attachment is compressed today; thumbnail/ICC blocks are stripped or copied as-is.
8. **Verify input checksums on read** — XFM writes SHA-1 checksums but does not validate an existing block's checksum on load; could warn on mismatch (corruption detection).
